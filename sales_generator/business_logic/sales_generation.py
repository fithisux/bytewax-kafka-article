...
def generate_sales(products: List[modeling.Product], generator_config: GeneratorConfig) -> List[modeling.Purchase]:
    ...
    purchases : List[modeling.Purchase] = []
    for _ in range(0, items):
        ...
        quantity = 1 + generate_choice_from_weights(generator_config.transaction_quantity_weights)
